...
function handleAPI() {
  return new Promise(function (resolve) {
    ...
    //   }
    // })
    xhr.open('GET', `https://my-json-server.typicode.com/yyhurs/json-server-db/posts`)
    ...
    xhr.onload = function () {
      ...
      resolve((allData = JSON.parse(xhr.responseText)))
    }
    xhr.send(allData)
  })
}
...
function renderContent(goPage) {
  ...
  async function getPageAPI() {
    ...
    //   }
    // })
    xhr.open('GET', `https://my-json-server.typicode.com/yyhurs/json-server-db/posts?_page=${page}&_limit=6`)
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded')
    xhr.setRequestHeader('cache-control', 'no-cache')
    xhr.setRequestHeader('Postman-Token', '4c996eea-ceb2-4a48-b3e6-c326b99ca109')
    xhr.send(data)
    xhr.onload = function () {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        li.innerHTML = `
                ...
                <a href="https://my-json-server.typicode.com/yyhurs/json-server-db/item.html?id=${data[i].id}" class="main__link flex-row justify-content-space-between js-job-link">
  ...
  getPageAPI().then(() => showPagination(allData))
}
function filter(goPage) {
  ...
  filterBtn.addEventListener('click', function () {
    ...
    let searched = allData.map((obj) => {
      if (Object.keys(obj).some((property) => obj[property].toString().indexOf(filterTxt.value) != -1)) return obj
    })
    ...
    for (var i = 0; i < result.length; i++) {
      ...
      li.innerHTML = `
                <li class="main__item" data-item="${result[i].id}">
                    <a href="https://my-json-server.typicode.com/yyhurs/json-server-db/item.html?id=${result[i].id}" class="main__link flex-row justify-content-space-between js-job-link">
                        <div class="main__tertiary-deatils flex-col justify-content-space-between">
                            <div class="details__logo">
...
function showPagination(allData) {
  if (!allData) return
  var pageLen =
    Math.floor(allData.length % 6) === 0
      ? allData.length / 6
      : Math.floor(allData.length / 6) === 0
      ? 1
      : Math.floor(allData.length / 6) + 1
